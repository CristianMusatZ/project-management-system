  ...
  return (
    <div className="p-5 space-y-5">
      ...
      <div className="animate-slide-in-up" style={{ animationDelay: '0ms' }}>
        ...
        <p className="text-gray-500 mt-1 text-sm">Iată o privire de ansamblu asupra proiectelor tale.</p>
      </div>

      {/* Carduri statistici — staggered fade-in */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {cards.map((card, idx) => {
          ...
          return (
            <div
              key={card.label}
              className="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-5
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-5
                        animate-slide-in-up card-hover" style={{ animationDelay: '350ms' }}>
          <h2 className="text-sm font-semibold text-gray-900 mb-4">Sarcini pe status</h2>
          {loading
            ? <div className="skeleton h-28 w-full" />
            : <DonutChart segments={tasksByStatus} />}
        </div>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-5
                        animate-slide-in-up card-hover" style={{ animationDelay: '420ms' }}>
          <h2 className="text-sm font-semibold text-gray-900 mb-4">Sarcini pe prioritate</h2>
          {loading
            ? <div className="skeleton h-28 w-full" />
            : <HorizontalBarChart data={tasksByPriority} />}
        </div>
      </div>

      {/* Timeline */}
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-5
                      animate-slide-in-up" style={{ animationDelay: '500ms' }}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-gray-900">Cronologie proiecte</h2>
          <Link to="/projects"
        ...
        {loading ? <div className="skeleton h-36 w-full" /> : <TimelineChart projects={projects} />}
      </div>

      {/* Liste rapide */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200/80 overflow-hidden
                        animate-slide-in-up" style={{ animationDelay: '580ms' }}>
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100 bg-gray-50/50">
            <h2 className="text-sm font-semibold text-gray-900">Sarcini recente</h2>
            <Link to="/tasks"
              className="text-xs font-medium text-primary-600 hover:text-primary-800 transition-colors
                         flex items-center gap-1 group">
              Toate
          ...
          {recentTasks.length === 0 ? (
            <p className="text-gray-400 text-center py-8 text-sm animate-fade-in">Nicio sarcină încă.</p>
          ) : (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200/80 overflow-hidden
                        animate-slide-in-up" style={{ animationDelay: '640ms' }}>
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100 bg-gray-50/50">
            <h2 className="text-sm font-semibold text-gray-900">Deadline depășit</h2>
            <Link to="/projects"
              className="text-xs font-medium text-primary-600 hover:text-primary-800 transition-colors
                         flex items-center gap-1 group">
              Toate
          ...
          {overdueProjects.length === 0 ? (
            ...
            <div className="divide-y divide-gray-50">
              {overdueProjects.map((proj, idx) => (
                <Link
                  key={proj._id}
                  to={`/projects/${proj._id}`}
                  className="flex items-center gap-3 px-5 py-3 hover:bg-red-50/40 transition-colors
                             animate-slide-in-up group"
                  style={{ animationDelay: `${660 + idx * 50}ms` }}
                >
                  <AlertTriangle className="w-4 h-4 text-red-500 flex-shrink-0 group-hover:scale-110 transition-transform duration-150" />
                  <div className="flex-1 min-w-0">
